...
class AIStrategyGenerator:
    ...
    def _assess_risk(self, profile: TargetProfile) -> dict:
        ...
        risky_techs = [Technology.WORDPRESS, Technology.DRUPAL, Technology.PHP, Technology.LARAVEL]
        ...
        if profile.target_type in [TargetType.WEB_APP, TargetType.SPA, TargetType.REST_API]:
            phases.append(
                ScanPhase(
                    ...
                    priority=2,
                    depends_on=(
                        ["reconnaissance"] if "reconnaissance" in [p.name for p in phases] else []
                    ),
                    config={
                        "test_xss": True,
                        "test_sqli": True,
        ...
        if profile.has_auth or profile.has_forms:
            phases.append(
                ScanPhase(
                    ...
                    priority=4,
                    depends_on=(
                        ["auth_testing"] if "auth_testing" in [p.name for p in phases] else []
                    ),
                    config={
                        "test_idor": True,
    ...
    def _generate_rationale(self, profile: TargetProfile, plan: ScanPlan) -> str:
        ...
        rationale_parts.append(f"Target type detected: {profile.target_type.value}. ")
