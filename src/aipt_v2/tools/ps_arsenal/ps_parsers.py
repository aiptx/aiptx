...
class PSArsenalParser:
    ...
    def _init_patterns(self):
        ...
        self.patterns = {
            ...
            "wlan_key": re.compile(r"Key Content\s*:\s*(.+?)(?:\n|$)", re.IGNORECASE),
            ...
            # Mimikatz patterns
            "mimikatz_user": re.compile(r"\*\s*Username\s*:\s*(.+?)(?:\n|$)", re.IGNORECASE),
            "mimikatz_domain": re.compile(r"\*\s*Domain\s*:\s*(.+?)(?:\n|$)", re.IGNORECASE),
            "mimikatz_password": re.compile(r"\*\s*Password\s*:\s*(.+?)(?:\n|$)", re.IGNORECASE),
            "mimikatz_ntlm": re.compile(r"\*\s*NTLM\s*:\s*([a-fA-F0-9]{32})", re.IGNORECASE),
            "mimikatz_sha1": re.compile(r"\*\s*SHA1\s*:\s*([a-fA-F0-9]{40})", re.IGNORECASE),
            # Kerberos tickets
            "krb_tgs": re.compile(r"\$krb5tgs\$[^\s]+"),
            "krb_asrep": re.compile(r"\$krb5asrep\$[^\s]+"),
            # VM detection
            "vm_detected": re.compile(
                r"(VMware|VirtualBox|Hyper-V|QEMU|Xen|Virtual\s*PC|KVM)", re.IGNORECASE
            ),
            # Browser credentials
            ...
            # LSA secrets
            "lsa_secret": re.compile(r"Secret\s*:\s*(.+?)(?:\n|$)", re.IGNORECASE),
            # Session info
    ...
    def parse_get_passhashes(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
    ...
    def parse_get_wlan_keys(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
        ...
        for i, ssid_match in enumerate(ssid_matches):
            ...
            if ssid:
                cred = PSCredential(
                    username=ssid, password=key, source="Get-WLAN-Keys", credential_type="wlan"
                )
                credentials.append(cred)
    ...
    def parse_invoke_mimikatz(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
    ...
    def parse_session_gopher(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
        ...
        for session in putty_matches:
            findings.append(
                PSFinding(
                    ...
                    description="Saved PuTTY session configuration found",
    ...
    def parse_web_credentials(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
    ...
    def parse_lsa_secret(self, output: str) -> Tuple[List[PSFinding], List[PSCredential]]:
