...
from aipt_v2.utils.searchers.Domain_Filter import code_white_list, repository_filter
from aipt_v2.utils.searchers.Extension_Filter import for_github_code_file, for_github_repo_file
...
class GithubSearcher:
    ...
    USE_PROXY = False
    proxies = {"http": "socks5://127.0.0.1:50532", "https": "socks5://127.0.0.1:50532"}
    ...
    def filter_items(self, items, keyword, size_limits, loose_mode=False):
        ...
        for item in items:
            ...
            if "cve" in keyword.lower():
                if result_matches_cve(keyword, name_temp) or result_matches_cve(
                    keyword, description_temp
                ):
        ...
        filtered_items = [
            ...
            if not any(filter_string in item["html_url"] for filter_string in repository_filter)
        ]
        ...
        for item in extracted_items:
            if item["description_length"] <= 300:
                ...
                d_score = c.times_0 * norm.pdf(
                    item["description_length"], loc=c.mus[0], scale=c.sigmas[0]
                )

            if item["open_issues_count"] <= 30:
                i_score = c.max_confs[1]
            else:
                i_score = c.times_1 * norm.pdf(
                    item["open_issues_count"], loc=c.mus[1], scale=c.sigmas[1]
                )
            ...
            item["conf_score"] = (
                d_score * c.conf_score_weights[0]
                + i_score * c.conf_score_weights[1]
                + t_score * c.conf_score_weights[2]
            )
            ...
            lamda = (
                c.times_2 * norm.pdf(days_difference, loc=c.mus[2], scale=c.sigmas[2]) + c.base_line
            )
            item["efct_score"] = lamda * item["stars_count"] / max(1, item["forks_count"])

        filtered_extracted_items = [
            item for item in extracted_items if item["conf_score"] >= c.threshold
        ]
        ...
        sorted_filtered_extracted_items = sorted(
            filtered_extracted_items, key=lambda x: x["efct_score"], reverse=True
        )
        # merge results
        ...
        group1 = [
            item
            for item in merged_filtered_items
            if c.size_limits[0] <= item["size"] <= c.size_limits[1]
        ]
    ...
    def _search_code(
        ...
        if self.USE_PROXY:
            resp = self.session.get(
                url, headers=header, proxies=self.proxies, timeout=30, stream=True
            )
        else:
        ...
        if "X-RateLimit-Remaining" in resp.headers.keys():
            self.search_limit_remaining = int(resp.headers["X-RateLimit-Remaining"])
        ...
        if query_type == "repositories":
            filtered_items, effective_count = self.filter_items(
                items, keyword, size_limits, loose_mode=mode
            )
            # update result
            result["effective_count"] = effective_count
        elif query_type == "code":
            filtered_items = [
                item for item in items if any(repo in item["html_url"] for repo in code_white_list)
            ]
            # update result
    ...
    def search_keyword(
        self, keyword: str, output_dir: str, filter_on: bool = True, loose_mode: bool = False
    ):
        # print("github called")
        ...
        mode = True if loose_mode else False
        repo_result, _ = self._search_code(
        ...
        if filter_on:
            code_result, _ = self._search_code(
                query_type="code",
                query_body=f'"{keyword}"',
                qualifiers="",
                keyword=keyword,
        ...
        with tqdm(
            total=repo_target_count, desc=f"Searching repositories related to {keyword} from GitHub"
        ) as pbar:
            try:
                for item in repo_result["items"]:
                    ...
                    repo_directory = os.path.join(
                        output_dir, f"{repo_star}_{repo_name}_{repo_language}"
                    )
                    if not os.path.exists(repo_directory):
                        ...
                        if filter_on:
                            ...
                            remove_files(
                                repo_directory,
                                filter_list=for_github_repo_file,
                                remove_no_extension=True,
                            )
                            file_count_1, _ = count_files_and_size(repo_directory)
                            if (
                                file_count_0 * 0.5 > file_count_1
                                or file_count_1 > c.file_num_limits
                            ):
        ...
        try:
            ...
            for item in repo_result["items"]:
                item.update(
                    {
                        key: item[key]
                        for key in ["name", "clone_url", "stars_count", "forks_count"]
                        if key in item
                    }
                )
            ...
            if (repo_result["total_count"] <= c.per_page) or (
                repo_result["total_count"] > c.per_page
                and repo_result["effective_count"] < c.per_page
            ):
                repo_result["trend_score"] = c.trend_weights[0] * repo_result[
                    "effective_count"
                ] + c.trend_weights[1] * (total_stars_count + total_forks_count)
            else:
                repo_result["trend_score"] = (
                    c.trend_weights[0] * repo_result["total_count"]
        ...
        with tqdm(
            total=code_target_count, desc=f"Downloading code files related to {keyword} from GitHub"
        ) as pbar:
            ...
            try:
                for item in code_result["items"]:
                    ...
                    if response.status_code == 200:
                        file_content, truncated = _read_capped_content(response, label=file_url)
                        if truncated:
